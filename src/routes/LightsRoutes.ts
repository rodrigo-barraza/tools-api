...
router.post(
  ...
  asyncHandler(
    async (req: Request) => {
      const { selector, hue, saturation, brightness, kelvin, duration } =
        req.body;
  ...
  asyncHandler(
    async (req: Request, res: Response) => {
      ...
      if (!Array.isArray(states) || states.length === 0) {
        return res
          .status(400)
          .json({ error: "states must be a non-empty array" });
      }
      return await LightsDataService.setStates(states, defaults);
    },
    "Set multi-light states",
    { errorStatus: 502, health },
  ),
);
...
router.post(
  ...
  asyncHandler(
    async (req: Request) => {
      const { selector, color, fromColor, period, cycles, persist, powerOn } =
        req.body;
        ...
        case "set":
          if (locked === undefined) {
            return res
              .status(400)
              .json({
                error: "locked (boolean) is required when action is 'set'",
              });
          }
