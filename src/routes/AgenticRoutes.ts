  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (!code || typeof code !== "string" || !code.trim()) {
      return res
        ...
        .json({
          error:
            "Request body must include 'code' (non-empty string — JavaScript to execute)",
        });
    }
    ...
    try {
      ...
      res
        .status(500)
        .json({ error: `Custom tool creation failed: ${errorMessage(error)}` });
    }
  }),
);
// ── List Custom Tools ─────────────────────────────────────────
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    try {
      ...
      res
        .status(500)
        .json({ error: `Custom tool list failed: ${errorMessage(error)}` });
    }
  }),
);
// ── Update Custom Tool ────────────────────────────────────────
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (!id || typeof id !== "string") {
      return res
        ...
        .json({
          error:
            "Request body must include 'id' (string — the tool's MongoDB ObjectId)",
        });
    }
    try {
      const prismRes = await fetch(
        `${CONFIG.PRISM_SERVICE_URL}/custom-tools/${id}`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
      ...
      res
        .status(500)
        .json({ error: `Custom tool update failed: ${errorMessage(error)}` });
    }
  }),
);
// ── Delete Custom Tool ────────────────────────────────────────
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    try {
      ...
      res
        .status(500)
        .json({ error: `Custom tool deletion failed: ${errorMessage(error)}` });
    }
  }),
);
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const result = executeJavaScript(wrappedCode, {
      timeout: 30_000,
      execution,
    });
