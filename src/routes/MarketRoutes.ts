...
router.get("/commodities/summary", (_req: Request, res: Response) => {
  res.json(getCommoditySummary());
});
router.get("/commodities/categories", (_req: Request, res: Response) => {
  res.json(Object.values(ASSET_CATEGORIES));
});
router.get("/commodities/category/:category", (req: Request, res: Response) => {
...
router.get("/commodities/ticker/:ticker", (req: Request, res: Response) => {
    ...
    if (!action)
      return res
        ...
        .json({
          ...
          actions: ["summary", "category", "ticker", "categories", "history"],
        });
    switch (action) {
      case "summary":
      ...
      case "category": {
        if (!category)
          return res
            .status(400)
            .json({ error: "'category' required for action=category" });
        const data = getCommoditiesByCategory(category.toLowerCase());
        return res.json({
          action,
          category,
          ...(Array.isArray(data)
            ? { count: data.length, commodities: data }
            : data),
        });
      }
      case "ticker": {
        if (!ticker)
          return res
            .status(400)
            .json({ error: "'ticker' required for action=ticker" });
      ...
      case "categories":
        return res.json({
          action,
          categories: Object.values(ASSET_CATEGORIES),
        });
      case "history": {
        if (!ticker)
          return res
            .status(400)
            .json({ error: "'ticker' required for action=history" });
