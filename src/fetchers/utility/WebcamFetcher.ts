...
export async function getPublicWebcams({
  ...
  if (!supportedCities.includes(normalizedCity)) {
    throw new Error(
      `Webcams for city '${city}' are not currently supported. Supported: ${supportedCities.join(", ")}`,
    );
  }
  ...
  if (isStale) {
    ...
    try {
      const refreshFunction =
        WEBCAM_REGISTRY[normalizedCity as keyof typeof WEBCAM_REGISTRY];
      if (refreshFunction) {
        await refreshFunction();
      }
    } catch (error: unknown) {
      logger.error(
        `Failed to refresh webcams for ${capitalizedCity}:`,
        errorMessage(error),
      );
