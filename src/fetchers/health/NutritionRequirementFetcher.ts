...
export function calculateTargetProfile({
  ...
  const lifeStageLower = (
    lifeStage || (speciesLower === "human" ? "adult_male" : "adult_maintenance")
  ).toLowerCase();
  const targetAuth = (
  ...
  if (baseRules.length === 0) {
    const combos = [
      ...new Set(
        REQUIREMENTS_DB.map(
          (r) => `${r.species}/${r.demographic_life_stage}/${r.authority}`,
        ),
      ),
    ].sort();
    return {
      error: `No requirement data for species="${speciesLower}", lifeStage="${lifeStageLower}", authority="${targetAuth}". Available species/lifeStage/authority combinations: ${combos.join(", ")}`,
    };
  }

  const requirements: Record<string, NutrientTargetMetrics> = {};
  const compositional: string[] = [];
  const skippedWeightScaled: string[] = [];
  ...
  for (const rule of baseRules) {
    ...
    if (rule.metric === "RDA_multiplier_per_kg") {
      // Without a body weight the per-kg value is not a daily target —
      // emitting it raw would give absurd targets (e.g. 14 mg leucine/day).
      if (!weightKg) {
        skippedWeightScaled.push(rule.nutrient_id);
        continue;
      }
    ...
    requirements[rule.nutrient_id][rule.metric] = {
  ...
  return {
    ...(skippedWeightScaled.length > 0
      ? {
          _hint: `${skippedWeightScaled.length} weight-scaled requirements (${[...new Set(skippedWeightScaled)].join(", ")}) were omitted — pass weightKg to include them.`,
        }
      : {}),
    _context: {
